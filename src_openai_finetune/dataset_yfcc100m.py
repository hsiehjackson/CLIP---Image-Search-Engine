...
class CLIPDataset(Dataset):
    ...
    def __getitem__(self,idx):
        ...
        if self.tokenizer:
            text = self.tokenizer(text).squeeze().tolist()
    ...
    def collate_fn(self, batch):
        ...
        text = torch.LongTensor([self.pad_to_len(b['text'], max_length) for b in batch])
        return {
            'image': image,
            'text': text  
        }
    ...
    def pad_to_len(self, seq, to_len):
...
class SampleDataset(Dataset):
    ...
    def __getitem__(self,idx):
        ...
        if self.tokenizer:
            text = self.tokenizer(text_raw).squeeze().tolist()
